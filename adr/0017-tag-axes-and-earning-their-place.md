# 0017: Tags live on two axes — function and entity-type; a tag earns its place by changing how you handle the note
...
[[0012]] already supplied two relevant pieces: tags **earn their place at critical mass** (a
...
`:structure:`/`:moc:` tag. What 0012 did not give is (a) a model of the *axes* the tag
...
- **Function / lifecycle axis** — `:fleeting: :literature: :permanent: :runbook:`. Answers:
  *do you open this note to **think** or to **do**?* (and, for the maturity tags, how far
  along it is). This is 0012's maturity ladder plus `:runbook:`.
...
- **Context/domain facets** (`:tulip: :adhd: :career:`) are a *third, faceted layer* —
  neither axis — and per [[0012]] stay sparse and earned at mass. They mark "what life-area
  this touches," not what the note is or what you do with it.
- **Hub-ness is not a third tag axis.** Per [[0012]], a structure note's hub *function* is
...
0012 gave the *quantity* gate (critical mass). This is the *quality* gate, and a candidate
...
- **Function-axis dodge** — a tag that sits on the function axis but *refuses to answer*
  ...
  an answer (`:runbook:` or `:permanent:`) is required. The deprecation recorded in [[0012]]
...
0012's mass gate (a real "show me all of them" retrieval need or a shared handling ritual).

### Entity hubs are a note species with adjusted criteria

The seven criteria of [[0012]] are calibrated for *claim atoms*. Entity/reference nodes
(`:person:`, `:business:`) play by adjusted rules:

- a **noun title is correct** — the entity's name *is* the retrieval key (0012 criterion 2,
...
- **atomicity is judged by *referent*, not *idea*** — [[0012]]'s atomicity-as-linkability
...
This is the entity-axis cousin of 0012's structure note — but with a deliberate asymmetry:
...
distinction) *and* the 0012 quantity gate (no demonstrated retrieval need). Revisit as a
...
**Pre-create a generic entity tag for the future** — rejected per [[0012]]: do not
...
- `brain/CLAUDE.md` should restructure its FILETAGS section to group tags by axis
  ...
  *handling distinction* (this ADR) **and** *critical mass* ([[0012]]) — that any new tag
  must pass.
- This ADR is the *structural* rationale behind 0012's `:resource:` → `:runbook:`/
  `:permanent:` deprecation: `:resource:` is a function-axis dodge.
- Pairs with [[0012]]: 0012 = the bar a note must clear + the quantity gate for tags; 0017 =
  the axes of the vocabulary + the quality gate. Relates to [[0015]] only in that the
